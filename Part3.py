...
class NeuralNetworkTrainer:
    ...
    def train_net_with_hyper_parameters(self, configs: HyperParameterConfigs):
        ...
        part3_nn = Part3NN(num_classes)
        part3_nn = part3_nn.to(device)

        part3_nn = part3_nn.to(device)

        optimizer = optim.Adam(params=part3_nn.parameters(), lr=lr, weight_decay=weight_decay)

        loss_function = nn.CrossEntropyLoss()

        lr_scheduler = optim.lr_scheduler.ExponentialLR(optimizer, gamma=.99)

        # train model
        fine_tuned_trained_net, train_statistics = self.train_model(part3_nn,
    ...
    def train_model(model, data_loaders, datasets,
        ...
        for epoch in range(epochs_number):
            ...
            for i, batch in enumerate(data_loaders[TRAIN_STR]):
                ...
                top1_corrects, top5_corrects = NeuralNetworkTrainer.top_k_corrects(outputs, true_labels, device=device)
            ...
            for i, batch in enumerate(data_loaders[VALIDATION_STR]):
                with torch.no_grad():
                    ...
                    top1_corrects, top5_corrects = NeuralNetworkTrainer.top_k_corrects(outputs, true_labels, device,(1, 5))
